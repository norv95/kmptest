...
class Authors extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('name', 'required'),
			array('name, secname, lastname', 'length', 'max'=>255),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('id, name, secname, lastname', 'safe', 'on'=>'search'),
		);
	}
	...
	public function relations()
	{
		...
		return array(
            'booksAuthors' => array(self::MANY_MANY,'BooksAuthors','books_authors(id, author_id)')
		);
	}
